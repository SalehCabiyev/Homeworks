...
#include <fstream>
#include <sstream>
#include <cmath>  // Для расчета расстояния
...
private:
    ...
    bool carAdded;
    std::vector<std::string> trips;  // Список поездок водителя

public:
    std::string password; // Пароль для водителя
...
// Функция для вычисления расстояния между двумя точками (для упрощения — евклидово расстояние)
    ...
    void loadData() {
        ...
        while (std::getline(inFile, line)) {
            ...
            if (type == "Driver:") {
                ...
                trips.push_back(line);  // Чтение поездок
            }
        }
    }
};
...
int main() {
    TaxiCompany company;
    company.loadData(); // Загружаем данные из файла
